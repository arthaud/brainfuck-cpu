...
# [r_write_2h] write on the 2 highest bytes of a register
# pre: i = index, d0=d1=0, [d2 d3] = value
#      the cursor is on i
# post: i=d0=d1=d2=d3=0
#       the cursor is on i
r_write_2h="$r_fill_index
            >>>>>>>[>>>>>>]>>>>[-]>[-]<<<<<<<<<<<[<<<<<<]
            >>>>[->>[>>>>>>]>>>>+<<<<<<<<<<[<<<<<<]>>>>]
            >[->[>>>>>>]>>>>>+<<<<<<<<<<<[<<<<<<]>>>>>]
            >[>>>>>>]
